...
export const createDiaryValidationSchema = {
  [Segments.BODY]: Joi.object({
    ...
    emotions: Joi.array().items(Joi.string().custom(objectIdValidator)).min(1).max(12).required(),
  }),
};
export const updateDiaryValidationSchema = {
  ...
  [Segments.BODY]: Joi.object({
    ...
    date: Joi.string().pattern(/^\d{4}-\d{2}-\d{2}$/),
    emotions: Joi.array().items(Joi.string().custom(objectIdValidator)).min(1).max(12),
  }).min(1),
};
